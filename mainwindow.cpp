...
void MainWindow::startRecording() {
	...
	if (!currentRecording) {
		...
		if (recFileInfo.exists()) {
			...
			if (!QFile::rename(
					recFileInfo.absoluteFilePath(), recFileInfo.absolutePath() + '/' + newname)) {
		...
		currentRecording.reset(new recording(
			recFilename.toStdString(), checkedStreams, watchfor, syncOptionsByStreamName, 1));
